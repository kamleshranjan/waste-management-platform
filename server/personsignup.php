...
if($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // by adding (array_push()) corresponding error unto $error1 array
  if (empty($name)) { array_push($error1, "Full name is required"); }
  if (empty($myemail)) { array_push($error1, "Email is required"); }
  if (empty($mypassword)) { array_push($error1, "Password is required"); }
  if ($mypassword != $mypassword2) {
	array_push($error1, "The two passwords do not match");
  }
  ...
  if ($count > 0) { 
    ...
    if ($user['email'] == $myemail)
      array_push($error1, "email already exists");
  }
  

  // Finally, register user if there are no error1 in the form
  else {  
      ...
      { // if user exists
        ...
        $sql = "INSERT into personsignup values ('$name','$myemail','$mypassword','$address','$contact','$ch1','$ch2','$latitude','$longitude')";
